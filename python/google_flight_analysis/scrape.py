...
class _Scrape:
    ...
    def __add__(self, other):
        ...
        if obj_type == 'one-way':
            ...
            assert datetime.strptime(self.date[1], date_format) < datetime.strptime(other.date[0],
                                                                                    date_format), "Dates are not in order. Make sure to provide them in increasing order in YYYY-MM-DD format."
            ...
            if self.origin[0] == other.origin[0]:
                if self.data.empty:
                    return Scrape(
                        ...
                        *self.unpack(
                            [[other.origin[i], date] for i, date in enumerate(other.date)] + [[other.dest[-1]]])
                    )
                else:
                    obj = Scrape(
                        *self.unpack([[self.origin[i], date] for i, date in enumerate(self.date)]),
    ...
    def _set_properties(self, *args):
        ...
        # round-trip
            ...
            assert datetime.strptime(args[2], date_format) < datetime.strptime(args[3],
                                                                               date_format), "Dates are not in order. Make sure to provide them in increasing order in YYYY-MM-DD format."
        ...
        # chain-trip, chain is component of 3s, check that last one is an actual date to not confuse w perfect
            ...
            for i in range(0, len(args), 3):
                ...
                if i > 0:
                    assert datetime.strptime(self._date[-1], date_format) < datetime.strptime(args[i + 2],
                                                                                              date_format), "Dates are not in order ({d1} > {d2}). Make sure to provide them in increasing order in YYYY-MM-DD format.".format(
                        d1=self._date[-1], d2=args[i + 2])
        ...
        # perfect-chain
            ...
            for i in range(2, len(args) - 1, 2):
                ...
                assert datetime.strptime(self._date[-1], date_format) < datetime.strptime(args[i + 1],
                                                                                          date_format), "Dates are not in order ({d1} > {d2}). Make sure to provide them in increasing order in YYYY-MM-DD format.".format(
                    d1=self._date[-1], d2=args[i + 1])
